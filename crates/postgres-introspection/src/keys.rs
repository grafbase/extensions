...
pub(crate) async fn introspect_database(
    ...
    for row in rows {
        ...
        let Some(column_id) = database_definition.get_table_column_id(table_id, row.get(3)) else {
            continue;
        };

        let constraint_id = match database_definition.get_unique_constraint_id(table_id, row.get(1)) {
            Some(id) => id,
            None => {
