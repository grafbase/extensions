...
async fn eq() {
    ...
    // Check that deleted users don't exist anymore
    let query = indoc! {r"
        query {
          user(lookup: { id: 1 }) { id }
        }
    ...
    // Check that the non-deleted user still exists
    let query = indoc! {r"
        query {
          user(lookup: { id: 2 }) { id name }
        }
    "};
...
async fn xxx_eq_not_returning() {
    ...
    // Check that deleted users are gone
    let query = indoc! {r"
        query {
