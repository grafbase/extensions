...
pub use r#type::{ColumnType, DatabaseType, ScalarType};
pub use walkers::{
    EnumVariantWalker, EnumWalker, KeyColumnWalker, KeyWalker, RelationWalker, TableColumnWalker, TableWalker, Walker,
};

pub fn generate(schemas: Vec<SubgraphSchema<'_>>) -> HashMap<String, DatabaseDefinition> {
    let mut result = HashMap::new();

    for schema in schemas {
        result.insert(schema.name().to_string(), Default::default());
    }

    for schema in schemas {
        for r#type in schema.type_definitions() {
            match r#type {
                TypeDefinition::Object(object_definition) => todo!(),
                TypeDefinition::Enum(enum_definition) => todo!(),
                _ => unreachable!(),
            }
        }
    }
...
impl DatabaseDefinition {
    ...
    pub fn push_unique_constraint_column(&mut self, unique_constraint_column: KeyColumn) -> KeyColumnId {
        let id = self.next_unique_constraint_column_id();
        self.key_columns.push(unique_constraint_column);
    ...
    fn next_unique_constraint_id(&self) -> KeyId {
        KeyId(self.keys.len() as u32)
    }

    fn next_unique_constraint_column_id(&self) -> KeyColumnId {
