...
pub(super) fn introspect(schema: &SubgraphSchema<'_>, database_definition: &mut DatabaseDefinition) {
    for r#type in schema.type_definitions() {
        ...
        if let Some(returning) = object_definition
            .directives()
            .find(|d| d.name() == "pgReturning")
            .and_then(|d| d.arguments::<PgReturning>().ok())
        {
            map_returning_type(&object_definition, returning, database_definition);
        }
    }
}

fn map_returning_type(
    object_definition: &ObjectDefinition<'_>,
    returning: PgReturning<'_>,
...
fn map_operations(
    ...
    for field in object_definition.fields() {
        ...
        let r#type = match r#type
            .directives()
            .find(|d| d.name() == "pgMutation")
            .and_then(|d| d.arguments::<PgMutation<'_>>().ok())
        {
