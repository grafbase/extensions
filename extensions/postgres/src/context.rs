...
use create_input::CreateInputIterator;
...
impl<'a> Context<'a> {
    ...
    pub(crate) fn create_input(&'a self, table: TableWalker<'a>) -> Result<CreateInputIterator<'a>, SdkError> {
        let iterator = CreateInputIterator::new(self, table)?;
        Ok(iterator)
    }
    ...
    pub(crate) fn unique_filter(self, table: TableWalker<'a>) -> Result<FilterIterator<'a>, SdkError> {
        let filter = self.field.arguments::<serde_json::Map<String, Value>>(self.arguments)?;
        let iterator = UniqueFilterIterator::new(self, table, filter);
