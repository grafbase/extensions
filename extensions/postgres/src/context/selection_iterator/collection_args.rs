    ...
    for (i, (column, value, direction)) in fields.iter().enumerate() {
        ...
        if i == max_id {
            if value.is_null() {
                ...
                match direction {
                    OrderDirection::Asc => {
                        filters.push(column.greater_than(value).into());
                    }
                    OrderDirection::Desc => {
                        let tree =
                            ConditionTree::Or(vec![column.clone().less_than(value).into(), column.is_null().into()]);
            ...
            filters.push(column.equals(value).into());
        }
    }
